...
**VNC is a bootstrap transport, not the primary path — and it's slow.** macOS's built-in screensharingd is the only way to remotely view a Mac before TCC permissions are granted, but it tops out around 5–20fps even on a local network: ZRLE encoding in screensharingd's CPU path is the bottleneck (Apple's own implementation, not something we can fix). Cloud Mac hosts publicly position screensharingd as too slow for daily use too — OakHost explicitly says so on their product page. Our server transcodes VNC frames into H.264 / H.265 via WebCodecs hardware decode, which helps the wire format but not the source rate.

We use VNC for ONE reason: it's the only path that works before Screen Recording is granted. The server starts over VNC so you can see the screen long enough to click the macOS permission dialogs. Once Screen Recording and Accessibility are granted (typically within 30 seconds of first launch), the server upgrades automatically to SCK capture and CGEvent input — that's the path this project ships, and it runs at full 60fps with hardware-accelerated frame production from the GPU compositor. VNC keeps running as a warm spare to avoid screensharingd-process-died lockouts, but it's not the experience.
